...
export default class IMOperator {
    ...
    onSimulateReceiveMsg(cbOk) {
        this._app.getIMHandler().sendMsg({
            content: {
                type: 'get-history',
        ...
        this._app.getIMHandler().setOnReceiveMessageListener({
            listener: (msg) => {
                ...
                msg.isMy = msg.msgUserId === wx.getStorageSync('userInfoInServer').id;
    ...
    async onSimulateSendMsg({ content }) {
        ...
        try {
            const { content: contentSendSuccess } = await this._app.getIMHandler().sendMsg({ content });
